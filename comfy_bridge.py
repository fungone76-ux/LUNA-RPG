...
from google.genai import types  # Importante per i Safety Settings
...
DEFAULT_I2V_OUTPUT_FPS = int(os.getenv("WAN_I2V_OUTPUT_FPS", "24"))
...
def get_gemini_prompt(context_description: str) -> str:
    ...
    Include gestione Safety Settings su BLOCK_NONE per evitare che Gemini
    blocchi descrizioni horror/adult, restituendo None.
    """
    # Prompt di fallback in caso di errore critico
    fallback_prompt = "subject. subtle continuous motion. locked camera. cinematic, natural motion."
    ...
    Rules:
    - Prefer subtle, continuous motion (breathing, small head turn, gentle hand movement).
    ...
    try:
        # Configurazione Safety permissiva per evitare blocchi su contenuti horror/adult
        safety_settings = [
        ...
        response = client.models.generate_content(
            model="gemini-2.0-flash",  # Puoi usare gemini-1.5-flash o gemini-2.5-pro se preferisci
            contents=context_description,
            config=types.GenerateContentConfig(
                system_instruction=sys_instruction,
                temperature=0.4,
                max_output_tokens=80,
                safety_settings=safety_settings,  # APPLICHIAMO I FILTRI PERMISSIVI
            ),
        )

        # CONTROLLO ANTI-CRASH: Se response.text è None o vuoto
        if not response.text:
            print(
                f"⚠️ Warning Gemini: Risposta vuota (Probabile blocco Safety nonostante i settings). Uso il tuo input come fallback.")
            # Se Gemini fallisce, usiamo direttamente l'input dell'utente invece del fallback generico
            return context_description if context_description else fallback_prompt
        ...
        print(f"⚠️ Errore Gemini: {e}. Uso il testo originale come fallback.")
        # In caso di errore API, usiamo il testo originale dell'utente
        return context_description if context_description else fallback_prompt
...
def _apply_i2v_overrides(
    ...
    for _node_id, node in workflow.items():
        ...
        # RIFE (fluidità) - scala ridotta per VRAM
        if ctype == "RIFE VFI":
...
def generate_video_from_image(
    ...
    eff_rife_clear = int(
        rife_clear_cache_after_n_frames) if rife_clear_cache_after_n_frames is not None else DEFAULT_RIFE_CLEAR_CACHE
