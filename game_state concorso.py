...
def create_initial_game_state(companion_name: str = "Luna") -> Dict[str, Any]:
    ...
    return {
        ...
        "location": "St. Jude's Academy - Classroom",

        # --- MODIFICA: Dizionario punteggi invece di float singolo ---
        "affinity_scores": {
            "Luna": 5,  # Prof
            "Stella": 10,  # Compagna
            "Maria": 2  # Bidella
...
def switch_companion_memory(game_state: Dict, new_name: str):
    ...
    if old_name in game_state["npc_storage"]:
        ...
        print(f"[MEMORY] Dati salvati per {old_name}")
    ...
    if new_name in game_state["npc_storage"]:
        ...
        print(f"[MEMORY] Dati caricati per {new_name}")
    else:
        game_state["companion_name"] = new_name
...
def build_state_summary_text(game_state: Dict) -> str:
    ...
    # --- MODIFICA: Visualizzazione Affinità Multipla ---
    aff = game_state.get("affinity_scores", {})
    if isinstance(aff, dict):
        aff_str = f"L={aff.get('Luna', 0)} S={aff.get('Stella', 0)} M={aff.get('Maria', 0)}"
    else:
        aff_str = str(aff)
